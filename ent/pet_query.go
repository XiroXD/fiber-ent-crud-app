...
import (
	...
	"github.com/XiroXD/fiber-ent-crud-app/ent/predicate"
...
func (pq *PetQuery) QueryOwner() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, pet.OwnerTable, pet.OwnerPrimaryKey...),
		)
...
func (pq *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if query := pq.withOwner; query != nil {
		if err := pq.loadOwner(ctx, query, nodes,
			func(n *Pet) { n.Edges.Owner = []*User{} },
			func(n *Pet, e *User) { n.Edges.Owner = append(n.Edges.Owner, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PetQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(pet.OwnerTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(pet.OwnerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(pet.OwnerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(pet.OwnerPrimaryKey[1]))
